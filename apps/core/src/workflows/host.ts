...
export class RunHost extends RpcTarget {
   ...
   * execution and the engine didn't stop the execution: a failure
   * workflow code caught is replayed on every later execution, and was
   * recorded when it happened; a step the engine stopped mid-way runs, or
   * replays, in the execution that goes on. A step that fails
